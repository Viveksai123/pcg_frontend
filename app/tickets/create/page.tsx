...
export default function CreateTicketPage() {
  ...
  const handleGetMLPrediction = async () => {
    ...
    try {
      ...
      setError('Failed to get AI prediction. Please check ML service is running.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        description: formData.description,
        priority: mlResult?.predictions?.priority?.predicted || formData.priority,
        category: mlResult?.predictions?.category?.predicted || 'Uncategorized',
        resolver_group: mlResult?.predictions?.resolver_group?.assigned_to || '',
        email: user?.email || '',
        historical_tickets: [],
        knowledge_base: [],
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Create New Ticket</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Description */}
          <Card className="bg-slate-800 border-slate-700 p-6">
            <label htmlFor="description" className="block text-sm font-medium text-slate-200 mb-3">
              Description
            </label>
            <textarea
              id="description"
            ...
            <p className="text-slate-400 text-sm mt-2">
              The more details you provide, the better our AI can categorize your issue
            </p>
          </Card>
                ...
                <div className="bg-slate-700 rounded-lg p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">
                      {((mlResult.predictions.category.confidence || 0) * 100).toFixed(1)}% confidence
                    </p>
                  </div>
                  ...
                  <div className="border-t border-slate-600 pt-3">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-300">Predicted Priority:</span>
                      <span className="text-lg font-bold text-amber-400">
                        {mlResult.predictions.priority.predicted}
                      </span>
                    </div>
                    <p className="text-slate-400 text-sm mt-1">
                      Impact: {mlResult.predictions.priority.impact} | Urgency: {mlResult.predictions.priority.urgency}
                    </p>
                  </div>
                  ...
                  <div className="border-t border-slate-600 pt-3">
                    ...
                    <p className="text-slate-400 text-sm mt-1">
                      {((mlResult.predictions.resolver_group.confidence || 0) * 100).toFixed(1)}% confidence
                    </p>
                  </div>
                  ...
                  {mlResult.rag_insights?.auto_response?.draft && (
                    <div className="border-t border-slate-600 pt-3">
                      <span className="text-slate-300 block mb-2">Suggested Resolution Draft:</span>
                  ...
                  {/* Proactive Insights */}
                  {mlResult.rag_insights?.proactive_insights?.insights && mlResult.rag_insights.proactive_insights.insights.length > 0 && (
                    <div className="border-t border-slate-600 pt-3">
                      <span className="text-slate-300 block mb-2">Insights:</span>
                      <ul className="space-y-2">
                        {mlResult.rag_insights.proactive_insights.insights.map((insight, idx) => {
                          // Handle both string and object formats
                          const insightText = typeof insight === 'string' 
                            ? insight 
                            : (insight as any)?.message || (insight as any)?.recommendation || JSON.stringify(insight);
                          
                          return (
                            <li key={idx} className="text-sm text-slate-400 flex items-start gap-2 bg-slate-600 bg-opacity-50 p-2 rounded">
